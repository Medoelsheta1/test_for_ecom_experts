# Frontend Take-Home For Ecom Experts 

Hi Ecom Experts team,

This project is my solution for the frontend take-home test.

I built a responsive React bundle builder with product variants, quantity controls, a live review panel, price calculations, accordion steps, and localStorage persistence.

## Run locally

### 1. Clone the repository with these steps

```bash
1- git clone https://github.com/Medoelsheta1/test_for_ecom_experts.git
2- npm install
3- npm start